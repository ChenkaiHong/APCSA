...
public class WordAnalysis
{
    ...
    private void readFile(String fileName) throws IOException
    {
        ...
        while (inFile.hasNext())
        {
            ...
            if (newWord != null && 0 < newWord.length())
            {
                ...
                binarySearchToPressWord(newWord);
            }
        }
        inFile.close();
    }

    private int binarySearchToPressWord(String txt)
    {
        if(words.size()==0)
        {
    ...
    */
    ...
    {
        double sumTopWords = 0;
        for(int i = 0; i<num;i++)
        {
            sumTopWords += words.get(i).getFrequency();

        }
        double sumWords = 0;
        for(int i = 0; i<words.size();i++)
        {
            sumWords += words.get(i).getFrequency();
        }
        double ratio = sumTopWords/sumWords;
        return ratio;
    }
    ...
    private void mergeWords(int low , int mid, int high)
    {
        ...
        for(int index = 0; index < high - low + 1; index++)
        {
            ...
            else if(words.get(findex).getWord().compareTo(words.get(sindex).getWord()) < 0)
            {
                merged.add(words.get(findex));
                findex++;
            }
    ...
    /**
     * Merge portions of the "words" list by the frequency 
     * in the Word objects.  This is a helper method for sortWordsHelper.
     *       
     * @param low    the smallest index to be used in this portion of the merge
     * @param mid    the start of the second half of the array to be considered
     ...
     *               the merge
     */
    private void mergeFrequencies(int low , int mid, int high)
    {
        ArrayList <Word> merged = new ArrayList<Word>();
        int findex = low;
        int sindex = mid;
    ...
    */
    ...
    {
        System.out.println("\n\nEnter option: ");
        System.out.println("\t 1 - Sort words in alphabetical order\n" +
            "\t 2 - Print all book data \n" +
            "\t 3 - Print sum of words \n" +
            "\t 4 - Print sum of top words \n" +
            "\t 5 - Print top words \n" +
            "\t 6 - Get word quotient \n" +
            "\t 7 - Get the number of unique words \n" +
            "\t 8 - Get the percentage of the word quotient \n" +
            "\t 9 - Search for a word\n" +
            "\t10 - Print words in alphabetical order\n" +
            "\t11 - Print words by frequency\n"+
            "\t12 - Sort frequency \n"+
            "\t13 - Quit \n");
    }

    /**
     * Gains the user input about their decision regarding what to do with the information
     * 
     * @return  true if the user is done with the system (if the user quits); otherwise,
     *          false
     */
    ...
    public boolean interactWithUser( )
    {
        ...
        {
            System.out.println("Total number of words is: "+ sumWords());
        }
        else if(choice == 4)
        {
            System.out.println("\n\tHow many top words do you want to sum?\n\t");
            int topWords = in.nextInt();
            System.out.println("Sum of the Top Words: " + sumTopWords(topWords));
        }
        else if(choice == 5)
        {
            System.out.println("\n\tHow many top words do you want to print?\n\t");
            int topWords = in.nextInt();
        ...
        {
            System.out.println("\n\tHow many top words do you want to find the quotient for?\n\t");
            int topWords = in.nextInt();
            System.out.println("Word Quotient is: " + getWordQuotient(topWords));
        }
        else if(choice == 7)
        {
            System.out.println("Number of Unique Words: " + getNumberOfUniqueWords());
        }
        else if(choice == 8)
        {
            System.out.println("\n\tHow many top words do you want to calculate the percent for?\n\t");
            int topWords3 = in.nextInt();
            System.out.println("Percentage: " + calculatePercentage(topWords3) + "%");
        }
        else if(choice == 9)
        {
            System.out.print("\n\tWhich word do you want to search for?\n\t");
            String wordFind = in.nextLine();
            int index = findWord(wordFind);
            if (index >= 0)
            {
                System.out.println(words.get(index));
            }
    ...
    public static void main (String [] args) throws IOException
    {
        ...
        while(!areWeDone)
        {
            areWeDone = author.interactWithUser(); 
        }
    }
}
